...
int32_t gyroPosOld[3][3];
int32_t gyroNegOld[3][3];
...
int32_t gyro_offset[3] = {0};  // gyro_offset value calibrated by Gyro_calibrate()
int32_t gyro_r[3] = {0};
int32_t prevTick;
/*
 * INITIALISATION
...
void IMU_GyroRead2(ICM20948 *dev, UART_HandleTypeDef *uart)
{   // return the change in value instead of current value
    ...
    for(i = 0; i < 3; i++){
		...
		// Median Filtering
		gyroNegOld[i][2] = gyroNegOld[i][1];
		gyroPosOld[i][2] = gyroPosOld[i][1];

		gyroNegOld[i][1] = gyroNegOld[i][0];
		gyroPosOld[i][1] = gyroPosOld[i][0];

		gyroNegOld[i][0] = gyroNeg;
		gyroPosOld[i][0] = gyroPos;

		if(gyroNegOld[i][1] <= gyroNegOld[i][0] && gyroNegOld[i][1] >= gyroNegOld[i][2])valNeg = gyroNegOld[i][1];
		else if(gyroNegOld[i][1] <= gyroNegOld[i][2] && gyroNegOld[i][1] >= gyroNegOld[i][0])valNeg = gyroNegOld[i][1];

		else if(gyroNegOld[i][0] <= gyroNegOld[i][2] && gyroNegOld[i][0] >= gyroNegOld[i][1])valNeg = gyroNegOld[i][0];
		else if(gyroNegOld[i][0] <= gyroNegOld[i][2] && gyroNegOld[i][0] >= gyroNegOld[i][1])valNeg = gyroNegOld[i][0];

		else if(gyroNegOld[i][2] <= gyroNegOld[i][0] && gyroNegOld[i][2] >= gyroNegOld[i][1])valNeg = gyroNegOld[i][2];
		else valNeg = gyroNegOld[i][2];

		if(gyroPosOld[i][1] <= gyroPosOld[i][0] && gyroPosOld[i][1] >= gyroPosOld[i][2])valPos = gyroPosOld[i][1];
		else if(gyroPosOld[i][1] <= gyroPosOld[i][2] && gyroPosOld[i][1] >= gyroPosOld[i][0])valPos = gyroPosOld[i][1];

		else if(gyroPosOld[i][0] <= gyroPosOld[i][2] && gyroPosOld[i][0] >= gyroPosOld[i][1])valPos = gyroPosOld[i][0];
		else if(gyroPosOld[i][0] <= gyroPosOld[i][2] && gyroPosOld[i][0] >= gyroPosOld[i][1])valPos = gyroPosOld[i][0];

		else if(gyroPosOld[i][2] <= gyroPosOld[i][0] && gyroPosOld[i][2] >= gyroPosOld[i][1])valPos = gyroPosOld[i][2];
		else valPos = gyroPosOld[i][2];

		dev->gyro[i] = ((double)((int32_t)valPos - (int32_t)valNeg -  gyro_offset[i])*0.01525878906f*elapsedMs/1000);
//		sprintf(sbuf, "%7d ", (int32_t)valPos - (int32_t)valNeg -  gyro_offset[i]);
//		HAL_UART_Transmit(uart, (uint8_t *)sbuf, 8, HAL_MAX_DELAY);
//
//		sprintf(sbuf, "%7d ", gyro_offset[i]);
//		HAL_UART_Transmit(uart, (uint8_t *)sbuf, 8, HAL_MAX_DELAY);
...
void Gyro_calibrate2(ICM20948 *dev, uint8_t ticks, UART_HandleTypeDef *uart)  // calibrate the offset of the gyro
// store the offset in int16_t gyro_offset[3]
{
    int32_t offset_local[3] = {0};
    int32_t gyroRaw;
    uint32_t gyroNeg, gyroPos;
    int16_t i, j;
    uint8_t sbuf[10], u8Buf[2] = {0};;
...
//	}
    ...
    for (i=0; i< 512; i++){
    	for(j = 0; j < 3; j++){
    			...
    			offset_local[j] = offset_local[j] + (int32_t)gyroPos - (int32_t)gyroNeg;

//    			sprintf(sbuf, "%7d ", offset_local[j]);
//    			HAL_UART_Transmit(uart, (uint8_t *)sbuf, 8, HAL_MAX_DELAY);
    	}
//		HAL_UART_Transmit(uart, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
...
//		offset_local = offset_local + (double)dev->gyro[2]/64.0;
    ...
    for(j=0; j<3; j++){
    	gyro_offset[j] = offset_local[j]>>9;
	}
    prevTick = 0;
